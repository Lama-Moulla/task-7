const tbody = document.querySelector(".products tbody")
const form = document.querySelector("#productForm")
const ProductName= document.querySelector("#productForm #ProductName")
const price = document.querySelector("#productForm #price")
const category = document.querySelector("#productForm #category")
const image = document.querySelector("#productForm #image")


let products = JSON.parse(localStorage.getItem("products")) || [
    {
        id : 1,
        ProductName : "laptop",
        Price : `686$`,
        Category : "electronic",
        Image : "https://encrypted-tbn3.gstatic.com/shopping?q=tbn:ANd9GcR_2TZLfzygBN5iL139hbVU9ruI_aS6-WFTiYNa72-UltcJW2vPSamxPkR5M1ncWf_ffHXspH7x0bmiC5WHKFsQYrZwITkBvGg4oaUMzdBG"
    },
    {
        id : 2,
        ProductName :"laptop",
        Price : `583$`,
        Category : "electronic",
        Image : "https://encrypted-tbn0.gstatic.com/shopping?q=tbn:ANd9GcSFHy7OxA9BusUQdDWKmJN2rKeuVPE7yTc6IIKEnB7lLJpn-2eS1KZR_75mA8Ow1he0tCN4HYlVR2ki9IgoAoddwYHSkcyexZ8ugphFQNaCzX3DxCGogNca"
    },
    {
        id : 3,
        ProductName :"gold jwellary",
        Price : `104500$`,
        Category : "clothes",
        Image : "https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcSOpC7u7xhtqdNdjF2KVaLlhvZPH3PT6zliuGuE6cspUdmzlQss9khm-vrNLn34IBM_yJfhPaEvNRzjJlKizezGhURvwZZbGViA9ScMo_tn5en4L-J4g6iAplZ0YCTEhnVy_SP5cjP4Ow&usqp=CAc"
    },
    {
        id : 4,
        ProductName :"gold jwellary",
        Price : `533$`,
        Category : "clothes",
        Image : "https://encrypted-tbn0.gstatic.com/shopping?q=tbn:ANd9GcTzowBwGRq4PMgxGkJT2H43HdDXZ2Oz5IvTZjialN2_fbkfkNWEAdWZgQDgJwmI5kJY8q7vDhtE74Xx2UxblgKSiIVamUBt_Xq2iFfqEkJNSDmbyW5P8hxuf7qMw1BaMHRk3mkbsdtjav0&usqp=CAc"
    },
    {
        id : 5,
        ProductName :"silver jwellary",
        Price : `53$`,
        Category : "clothes",
        Image : "data:image/webp;base64,"
    },
    {
        id : 6,
        ProductName :"laptop",
        Price : `333.84$`,
        Category : "electronic",
        Image : "data:image/jpeg;base64,"
    }
] 

if (!localStorage.getItem("products")) {
    localStorage.setItem("products", JSON.stringify(products))
    localStorage.setItem("lastId", Math.max(...products.map(p => p.id)))
}

let lastId = parseInt(localStorage.getItem("lastId")) || 0

const read = () => {
    tbody.innerHTML = ""
    products.forEach((product) => {
        // console.log(products);
        
        tbody.innerHTML  += 
        `<tr>
            <td>
                <span id="product-${product.id}">${product.ProductName}</span>
                <form id="updateForm1-${product.id}" onSubmit="edit(event, ${product.id})">
                    <input type="text" placeholder="product" name="title" value="${product.ProductName}">
                    <input type="submit" value="edit" class="btn">
                </form>
            </td>
            <td>
                <span id="price-${product.id}">${product.Price}</span>
                <form id="updateForm2-${product.id}" onSubmit="edit(event, ${product.id})">
                    <input type="text" placeholder="price" name="price" value="${product.Price}">
                    <input type="submit" value="edit" class="btn">
                </form>
            </td>
            <td>
                <span id="category-${product.id}">${product.Category}</span>
                <form id="updateForm3-${product.id}" onSubmit="edit(event, ${product.id})">
                    <select name="category">
                        <option value="clothes" ${product.Category == "clothes" ? "selected" : ""}>clothes</option>
                        <option value="electronic" ${product.Category == "electronic" ? "selected" : ""}>electronic</option>
                    </select>
                    <input type="submit" value="edit" class="btn">
                </form>
            </td>
            <td class="tdImage">
                <span id="image-${product.id}" class="spanImage">${product.Image}</span>
                <form id="updateForm4-${product.id}" onSubmit="edit(event, ${product.id})">
                    <input type="text" placeholder="changeImage" name="image" value="${product.Image}">
                    <input type="submit" value="edit" class="btn">
                </form>
            </td>
            <td><button onclick = "handelEdit(${product.id})" class="btn">edit</button><button onclick="deleteProduct(${product.id})" class="btn">delete</button></td>
        </tr>`
    })
}

const add = (event) =>{
    event.preventDefault()
    const product = {
        id : lastId + 1,
        ProductName : ProductName.value,
        Price : price.value,
        Category : category.value,
        Image : image.value
    }
    products.push(product)
    /* ProductName.value = ""
    price.value = ""
    category.value = ""
    image.value = "" */
    form.reset()
    
    lastId = product.id

    localStorage.setItem("products", JSON.stringify(products))
    localStorage.setItem("lastId", lastId)
    read()
}

const handelEdit = (id) => {
    const imageColumn = document.querySelector(`thead .imageColumn`)
    imageColumn.style.display = "block"

    products.forEach(product => {
        const tdImage = document.querySelector(`#image-${product.id}`).parentElement
        const span4 = document.querySelector(`#image-${product.id}`)
        const form4 = document.querySelector(`#updateForm4-${product.id}`)
        span4.style.display = "block"
        form4.style.display = "none"
        tdImage.style.display = "block"
    })

    const span1 = document.querySelector(`#product-${id}`)
    const form1 = document.querySelector(`#updateForm1-${id}`)
    // console.log(span1, form1)
    span1.style.display = "none"
    form1.style.display = "block"

    const span2 = document.querySelector(`#price-${id}`)
    const form2 = document.querySelector(`#updateForm2-${id}`)
    // console.log(span2, form2)
    span2.style.display = "none"
    form2.style.display = "block"

    const span3 = document.querySelector(`#category-${id}`)
    const form3 = document.querySelector(`#updateForm3-${id}`)
    // console.log(span3, form3)
    span3.style.display = "none"
    form3.style.display = "block"

    const tdImage = document.querySelector(`#image-${id}`).parentElement
    const span4 = document.querySelector(`#image-${id}`)
    const form4 = document.querySelector(`#updateForm4-${id}`)
    span4.style.display = "none"
    form4.style.display = "block"
    tdImage.style.display = "block"
    tdImage.style.overflow = "hidden"
}


const edit = (event, id ) => {
    event.preventDefault()
    const newTitle = document.querySelector(`#updateForm1-${id} input[name="title"]`).value
    const newPrice = document.querySelector(`#updateForm2-${id} input[name="price"]`).value
    const newCategory = document.querySelector(`#updateForm3-${id} select[name="category"]`).value
    const newImage = document.querySelector(`#updateForm4-${id} input[name="image"]`).value

    products = products.map((product) => {
        if(product.id == id){
            product.ProductName = newTitle
            product.Price = newPrice
            product.Category = newCategory
            product.Image = newImage
        }
        return product
    })

    localStorage.setItem("products", JSON.stringify(products))
    read() 

    const imageColumn = document.querySelector(`thead .imageColumn`)
    const tdImages = document.querySelectorAll(`tbody .tdImage`)
    imageColumn.style.display = "none"
    tdImages.forEach(td => td.style.display = "none")
}

const deleteProduct = (id) => {
    products = products.filter((product) => {return product.id != id})
    localStorage.setItem("products", JSON.stringify(products))
    read()
}

form.addEventListener("submit", (event) => add(event))
read()



